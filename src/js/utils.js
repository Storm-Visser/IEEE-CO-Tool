...
// Stuff for verifying card rules? Might not be working...
export const verifyRules = async(t, card, list) => {
    ...
    const newChecklist = await queueTrelloRequest({
        method: 'GET',
        url: `${BASE_URL}cards/${card.id}/checklists?key=${appKey}&token=${token}`
    })
            ...
            if (!faultyChecklist) {
                ...
                try {
                    ...
                    faultyCheckItems.forEach(checkItem => {
                        promises.push(queueTrelloRequest({
                            method: 'DELETE',
                            url: `
                    ...
                    const addCheckItems = requirements.checkItems.filter(item => (!correctItemNames.includes(item.name)))
