...
// Updates relative due dates and basic card info in out database when the trello cards changes.
...
// Updates all children of card recursively
        ...
        currentCard.children.forEach(async childName => {
            ...
            childCards.forEach(async childCard => {
                if (childCard.parent === currentCard.cardName) { // ensure the card actually has the correct parent
                    ...
                    if (!childMoment.isValid()) { childMoment = null } else { childMoment = childMoment.toISOString() }
